...
function navigate() {
  ...
  // User is logged in — show sidebar and topbar
  authSection.style.display = 'none';
  appNavbar.style.display = 'flex';
  if (sidebar) sidebar.style.display = 'flex';
  
  // Update topbar
  const initials = activeUser.username.substring(0, 2).toUpperCase();
  document.getElementById('navUsername').innerText = activeUser.username;
  document.getElementById('navRole').innerText = activeUser.role === 'manager' ? 'Manager' : 'Client';
  ...
  if (sidebarUsername) sidebarUsername.innerText = activeUser.username;
  if (sidebarRole) sidebarRole.innerText = activeUser.role === 'manager' ? 'System Manager' : 'Personal Account';

  // Show/hide nav items
  const navNewAccount = document.getElementById('navNewAccount');
  const navDeposit = document.getElementById('navDeposit');
  const navTransfer = document.getElementById('navTransfer');
  if (navNewAccount) navNewAccount.style.display = activeUser.role === 'manager' ? 'none' : 'flex';
  if (navDeposit) navDeposit.style.display = activeUser.role === 'manager' ? 'none' : 'flex';
  if (navTransfer) navTransfer.style.display = activeUser.role === 'manager' ? 'none' : 'flex';
...
async function loadClientDashboard() {
  ...
  try {
    ...
    list.forEach(acc => {
      ...
      const statusClass = `status-${(acc.status || 'PENDING').toLowerCase()}`;
      const statusLabel = acc.status || 'PENDING';
      
      card.innerHTML = `
        <div class="account-card-top">
          <div class="account-card-icon">
            <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg>
          </div>
          <span class="account-status-badge ${statusClass}">${statusLabel}</span>
        </div>
        <div>
          <div class="account-name">${acc.name}</div>
          <div class="account-bankid">${acc.bankId || acc.id.substring(0, 8)}</div>
        </div>
        <div class="account-balance">${formatCurrency(acc.balance)}</div>
        <div class="account-card-actions">
          <button class="btn btn-teal" onclick="openTxModal('${acc.id}', '${acc.name}', 'deposit')">
            <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2.5" fill="none"><path d="M12 5v14M5 12l7 7 7-7"/></svg>
            Deposit
          </button>
          <button class="btn btn-ghost" onclick="openTxModal('${acc.id}', '${acc.name}', 'withdraw')">
            <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2.5" fill="none"><path d="M12 19V5M5 12l7-7 7 7"/></svg>
            Withdraw
          </button>
          <button class="btn btn-slate" onclick="openActivityLog('${acc.id}', '${acc.name}', ${acc.balance})">
            <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2.5" fill="none"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
            History
          </button>
          <button class="btn btn-ghost" onclick="openEditAccountName('${acc.id}', '${acc.name}')" title="Edit name">
            <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none"><path d="M12 20h9M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
          </button>
          <button class="btn btn-danger" onclick="handleDeleteAccount('${acc.id}', '${acc.name}')">
            <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2.5" fill="none"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
          </button>
        </div>
      `;
...
window.addEventListener('load', () => {
  ...
  if (navNewAccount) navNewAccount.addEventListener('click', (e) => {
    e.preventDefault();
    openModal('createAccountModal');
  });
  if (navDeposit) navDeposit.addEventListener('click', (e) => {
    e.preventDefault();
    openDepositModal();
  });
  if (navTransfer) navTransfer.addEventListener('click', (e) => {
    e.preventDefault();
    openTransferFunds();
  });
});
